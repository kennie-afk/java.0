...
export default function PropertyLocationPicker({ latitude, longitude, onChange }: Props) {
  ...
  if (!API_KEY) {
    return (
      <div className="h-56 rounded-lg bg-gray-50 dark:bg-white/5 border border-dashed border-base flex flex-col items-center justify-center gap-2 text-center px-4">
        <MapPinOff size={22} className="text-gray-400"/>
        <p className="text-sm text-muted">Map picker unavailable</p>
        <p className="text-2xs text-muted">Set NEXT_PUBLIC_GOOGLE_MAPS_API_KEY to enable</p>
      </div>
    )
  }

  if (!isLoaded) {
    return <div className="h-56 rounded-lg bg-gray-100 dark:bg-white/5 animate-pulse"/>
  }
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted">Click the map or drag the pin to set the exact location</p>
