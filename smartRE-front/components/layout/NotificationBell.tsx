  ...
  return (
    <div className="relative">
      <button
        ...
        <Bell size={17}/>
        {unread > 0 && (
          <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-1 rounded-full bg-red-500 text-white text-[10px] font-bold leading-4 text-center tabular-nums">
            {unread > 99 ? '99+' : unread}
          </span>
        )}
      </button>
          ...
          <div className="absolute right-0 top-[calc(100%+8px)] w-[340px] sm:w-[380px] card z-50 py-0 animate-fade-in overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-base">
              ...
              {unread > 0 && (
                <button onClick={onMarkAll}
                  className="flex items-center gap-1 text-[11px] text-gold-600 dark:text-gold-400 hover:underline">
            ...
            <div className="max-h-[380px] overflow-y-auto">
              {loading ? (
                ...
                <div className="px-4 py-10 text-center">
                  ...
                  <p className="text-[11px] text-muted mt-1">Updates about your listings, payments and verification land here.</p>
                </div>
              ) : shown.map(n => (
                <button key={n.id} onClick={() => onItemClick(n)}
                  className={cn(
                    'w-full text-left px-4 py-3 border-b border-base last:border-b-0 transition-colors',
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className={cn('text-[13px] leading-snug truncate',
                        n.read ? 'text-gray-600 dark:text-gray-300' : 'font-semibold text-gray-900 dark:text-white')}>
                        {n.subject || n.templateCode}
                      </p>
                      {n.body && <p className="text-[11.5px] text-muted mt-0.5 line-clamp-2">{n.body}</p>}
                      <p className="text-[10.5px] text-muted mt-1">{fmt.ago(n.createdAt)}</p>
                    </div>
                  </div>
                </button>
              ))}
            ...
            <div className="flex items-center justify-between px-4 py-2 border-t border-base bg-gray-50/60 dark:bg-white/[0.02]">
              <Link href="/notifications" onClick={() => setOpen(false)}
                className="text-[12px] font-medium text-gold-600 dark:text-gold-400 hover:underline">
                View all
              </Link>
              <Link href="/notifications#preferences" onClick={() => setOpen(false)}
                className="flex items-center gap-1 text-[11px] text-muted hover:text-gray-700 dark:hover:text-gray-200">
