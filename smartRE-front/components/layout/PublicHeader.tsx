    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-13 flex items-center gap-3" style={{ height: 52 }}>
        <Link href="/" className="flex items-center gap-1.5 shrink-0">
          <Logo size={28} idSuffix="header"/>
        </Link>

        <nav className="hidden lg:flex items-center gap-0.5 ml-3">
          {links.map(l => (
            <Link key={l.label} href={l.href}
              className="px-2.5 py-1.5 rounded-md text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-gray-900 dark:hover:text-white transition-colors">
              {l.label}
            </Link>
          ))}
        </nav>
        ...
        <Link href="/properties" className="hidden md:flex items-center gap-1.5 h-7 px-3 rounded-full border border-base text-sm text-muted hover:border-gold-300 hover:text-gray-700 dark:hover:text-gray-200 transition-colors">
        ...
        {user ? (
          <div className="relative shrink-0">
            <button onClick={() => setMenu(m => !m)} className="flex items-center gap-1.5 h-7 pl-1 pr-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">
              <div className="w-6 h-6 rounded-full bg-gold-500 text-white text-2xs font-bold flex items-center justify-center">{fmt.initials(user.fullName)}</div>
              ...
              <>
                ...
                <div className="absolute right-0 top-[calc(100%+8px)] w-48 card z-50 py-1 animate-fade-in">
                  ...
                  <Link href={user.role === 'ADMIN' ? '/overview' : '/dashboard'} onClick={() => setMenu(false)}
                    className="flex items-center gap-2 px-3 py-2 text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                    <LayoutDashboard size={13}/>{user.role === 'ADMIN' ? 'Admin console' : 'Dashboard'}
                  </Link>
                  <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors border-t border-base mt-1">
        ...
        <div className="lg:hidden border-t border-base bg-white dark:bg-[#0B0B18] px-4 py-2.5 space-y-0.5 animate-fade-in">
          {links.map(l => (
            <Link key={l.label} href={l.href} onClick={() => setOpen(false)}
              className="block px-2.5 py-2 rounded-md text-base font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5">
              {l.label}
            </Link>
          ))}
        </div>
