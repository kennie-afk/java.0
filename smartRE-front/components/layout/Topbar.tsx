      ...
      {user && (
        <div className="relative">
          <button onClick={() => setMenu(m => !m)}
            ...
            <div className="w-7 h-7 rounded-full bg-gold-500 text-white text-xs font-bold flex items-center justify-center shrink-0">
              {fmt.initials(user.fullName)}
            </div>
            <span className="text-base font-medium text-gray-700 dark:text-gray-200 hidden sm:block">{user.fullName.split(' ')[0]}</span>
              ...
              <div className="absolute right-0 top-[calc(100%+8px)] w-56 card z-50 py-1 animate-fade-in">
                ...
                {[
                  ...
                  <Link key={i.label} href={i.href} onClick={() => setMenu(false)}
                    className="flex items-center gap-2.5 px-4 py-2 text-base text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#1A1A35] hover:pl-[18px] transition-all duration-150">
                    <i.icon size={14}/>{i.label}
                  </Link>
                ))}
                <div className="border-t border-base mt-1">
                  <button onClick={handleLogout}
                    className="w-full flex items-center gap-2.5 px-4 py-2 text-base text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 hover:pl-[18px] transition-all duration-150">
