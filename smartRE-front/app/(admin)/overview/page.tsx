...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p className="text-2xs font-semibold uppercase tracking-widest text-gray-400 dark:text-gray-600 px-0.5">{children}</p>
}
  ...
  return (
    <Card className={cn('border-l-[3px] card-hover', tones.border)}>
      <div className="flex items-start justify-between gap-2 mb-2.5">
        ...
        <Link href={href} className="text-xs text-gold-500 hover:text-gold-600 font-medium flex items-center gap-0.5 mt-1">Review <ArrowUpRight size={11}/></Link>
      </div>
      <p className="text-2xl font-semibold text-gray-900 dark:text-white leading-none tabular-nums">{value}</p>
      <p className="text-xs font-medium text-gray-700 dark:text-gray-300 mt-2">{label}</p>
    ...
    <div className="space-y-7 pb-10">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted mt-1">{users.length} users · {properties.length} listings · {fmt.currency(summary?.totalPlatformFees || 0)} lifetime revenue</p>
        </div>
        <div className="relative w-full sm:w-72">
          <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/>
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search users, properties, counties..."
            className="w-full h-9 pl-8 pr-3 rounded-lg bg-gray-50 dark:bg-white/5 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gold-500/20 border border-base focus:border-gold-500 transition-all"/>
          {searchResults && (searchResults.users.length > 0 || searchResults.properties.length > 0) && (
            <div className="absolute right-0 top-[calc(100%+6px)] w-80 card z-50 py-1 max-h-80 overflow-auto animate-fade-in">
              {searchResults.users.length > 0 && (
                <div className="px-3 pt-2 pb-1 text-2xs font-semibold uppercase tracking-widest text-gray-400">Users</div>
              )}
              {searchResults.users.map(u => (
                <Link key={u.id} href="/users" className="flex items-center gap-2 px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-white/5">
                  <div className="w-6 h-6 rounded-full bg-gold-500 text-white text-2xs font-bold flex items-center justify-center shrink-0">{fmt.initials(u.fullName)}</div>
                  <span className="truncate">{u.fullName}</span><span className="text-muted ml-auto shrink-0">{u.role}</span>
                </Link>
              ))}
              {searchResults.properties.length > 0 && (
                <div className="px-3 pt-2 pb-1 text-2xs font-semibold uppercase tracking-widest text-gray-400 border-t border-base mt-1">Properties</div>
              )}
              {searchResults.properties.map(p => (
                <Link key={p.id} href={`/properties/${p.id}`} className="flex items-center gap-2 px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-white/5">
      ...
      <div className="space-y-2.5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <StatCard label="Verified users" value={`${stats.verifiedPct}%`} sub={`${stats.buyers} buyers · ${stats.sellers} sellers`} icon={<ShieldCheck size={16}/>} color="emerald"/>
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          <Card className="lg:col-span-2">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-display font-semibold text-base text-gray-900 dark:text-white flex items-center gap-2"><TrendingUp size={14} className="text-muted"/>Revenue trend</h2>
              <span className="text-xs text-muted">Platform fees by month</span>
            </div>
          ...
          <Card>
            <h2 className="font-display font-semibold text-base text-gray-900 dark:text-white flex items-center gap-2 mb-3"><PieChartIcon size={14} className="text-muted"/>Revenue split</h2>
      ...
      <div className="space-y-2.5">
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          <Card>
            <h2 className="font-display font-semibold text-base text-gray-900 dark:text-white mb-3">Listings by property type</h2>
            ...
            <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2 text-2xs text-muted">
          ...
          <Card>
            <h2 className="font-display font-semibold text-base text-gray-900 dark:text-white mb-3">Top counties</h2>
            <div className="space-y-2.5">
              {stats.topCounties.length === 0 && <p className="text-xs text-muted">No listings yet</p>}
              {stats.topCounties.map(([county, count]) => (
                <div key={county} className="flex items-center gap-2">
                  <span className="text-sm text-gray-700 dark:text-gray-300 w-20 truncate">{county}</span>
          ...
          <Card>
            <h2 className="font-display font-semibold text-base text-gray-900 dark:text-white flex items-center gap-2 mb-3"><Star size={14} className="text-muted"/>Top sellers by payout</h2>
            <div className="space-y-2.5">
              {stats.topSellers.length === 0 && <p className="text-xs text-muted">No payouts yet</p>}
      ...
      <div className="space-y-2.5">
        ...
        <Card padding="none">
          <div className="flex items-center justify-between p-4 pb-0">
            <h2 className="font-display font-semibold text-base text-gray-900 dark:text-white">Recent transactions</h2>
            <Link href="/revenue" className="text-xs text-gold-500 hover:text-gold-600 flex items-center gap-0.5 font-medium">Full ledger <ArrowUpRight size={11}/></Link>
          </div>
          <div className="divide-y divide-gray-100 dark:divide-[#1E1E3A] mt-3">
            {revenue.slice(0, 8).map(r => (
              <div key={r.id} className="flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-gray-50/60 dark:hover:bg-white/[0.02] transition-colors">
            ...
            {revenue.length === 0 && <p className="text-center text-sm text-muted py-8">No transactions recorded yet</p>}
          </div>
        </Card>
      </div>
    </div>
