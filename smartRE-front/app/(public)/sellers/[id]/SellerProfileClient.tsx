...
export default function SellerProfileClient() {
  ...
  if (notFound || !seller) {
    return (
      <div className="max-w-md mx-auto text-center py-12 px-4">
        <ShieldX size={22} className="text-gray-300 mx-auto mb-4"/>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 space-y-5">
      <Link href="#" onClick={() => router.back()} className="inline-flex items-center gap-1.5 text-base text-muted hover:text-gray-900 dark:hover:text-white">
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 gap-2.5 mt-4">
          <div className={`rounded-md border px-3 py-2.5 flex items-center gap-2 ${trust?.identityVerified ? 'border-emerald-200 dark:border-emerald-500/30 bg-emerald-50/50 dark:bg-emerald-500/5' : 'border-red-200 dark:border-red-500/30 bg-red-50/50 dark:bg-red-500/5'}`}>
            ...
            <div>
              <p className="text-xs font-semibold text-gray-900 dark:text-white">National ID + KRA</p>
              <p className="text-2xs text-muted">{trust?.identityVerified ? 'Verified' : 'Not verified'}</p>
            </div>
          </div>
          <div className={`rounded-md border px-3 py-2.5 flex items-center gap-2 ${trust?.ownershipVerified ? 'border-emerald-200 dark:border-emerald-500/30 bg-emerald-50/50 dark:bg-emerald-500/5' : 'border-amber-200 dark:border-amber-500/30 bg-amber-50/50 dark:bg-amber-500/5'}`}>
            ...
            <div>
              <p className="text-xs font-semibold text-gray-900 dark:text-white">Ardhisasa title check</p>
              <p className="text-2xs text-muted">{trust?.ownershipVerified ? 'Verified' : 'Pending / per listing'}</p>
            </div>
          </div>
        </div>
      </Card>
      ...
      {!hasAccess && pendingPayment && (
        <Card className="text-center">
          ...
          <h2 className="font-display font-semibold text-gray-900 dark:text-white text-lg mb-1">Payment pending</h2>
      ...
      {!hasAccess && !pendingPayment && !checkingPending && (
        <Card className="text-center">
          ...
          <h2 className="font-display font-semibold text-gray-900 dark:text-white text-lg mb-1">Unlock full seller details</h2>
          <p className="text-sm text-muted max-w-sm mx-auto mb-4">
          ...
          {user ? (
            user.role === 'BUYER' ? (
              ...
              <p className="text-sm text-muted">Sign in with a buyer account to unlock seller details.</p>
            )
          ) : (
      ...
      {hasAccess && (
        <Card>
          <h2 className="font-display font-semibold text-lg mb-3 flex items-center gap-1.5"><FileCheck size={15} className="text-gold-500"/>Contact details</h2>
      ...
      {listings.length > 0 && (
        <div>
          <h2 className="font-display font-semibold text-lg text-gray-900 dark:text-white mb-3">Active listings</h2>
      ...
      <Card>
        <h2 className="font-display font-semibold text-lg mb-3">Reviews</h2>
        {reviews.length === 0 ? (
          ...
          <div className="space-y-3">
            {(hasAccess ? reviews : reviews.slice(0, 1)).map(r => (
              <div key={r.id} className="border-b border-base last:border-0 pb-3 last:pb-0">
                <div className="flex items-center gap-2 mb-1"><RatingStars rating={r.rating}/><span className="text-xs text-muted">{fmt.ago(r.createdAt)}</span></div>
                {r.comment && <p className="text-sm text-gray-600 dark:text-gray-300">{r.comment}</p>}
              </div>
            ))}
            {!hasAccess && reviews.length > 1 && (
              <p className="text-xs text-muted text-center pt-1">{reviews.length - 1} more review{reviews.length - 1 === 1 ? '' : 's'} hidden. Unlock the full profile to see all</p>
            )}
          </div>
        )}
      </Card>
      ...
      <Modal open={payModal} onClose={() => setPM(false)} title="Unlock seller profile"
        ...
        <div className="space-y-3.5">
          <p className="text-base text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 rounded-md p-3">
            KES {accessFee} via M-Pesa. Access is granted the moment payment confirms and stays unlocked for this seller going forward.
          </p>
          <p className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10 rounded-md p-3">
