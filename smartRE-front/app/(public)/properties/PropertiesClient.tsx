  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
      <div className="mb-4">
        ...
        <p className="text-sm text-muted mt-0.5">{total.toLocaleString()} verified listings{keyword ? ` matching "${keyword}"` : ''}</p>
      </div>
      ...
      <div className="card p-2.5 flex flex-col sm:flex-row gap-2 mb-3.5">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full h-8 pl-8 pr-3 rounded-md bg-gray-50 dark:bg-white/5 border border-transparent text-base placeholder:text-gray-400 transition-all focus:outline-none focus:ring-2 focus:ring-gold-500/20 focus:border-gold-500 focus:bg-white dark:focus:bg-white/10"/>
        </div>
      ...
      {showFilters && (
        <div className="card p-2.5 mb-4 animate-fade-in space-y-3">
          ...
          <label className="flex items-center gap-2 text-sm p-2 rounded-md border border-base cursor-pointer w-fit">
      ...
      {total > PAGE_SIZE && (
        <div className="flex items-center justify-center gap-2 mt-6">
          ...
          <span className="text-sm text-muted px-2">Page {page + 1} of {Math.max(1, Math.ceil(total / PAGE_SIZE))}</span>
