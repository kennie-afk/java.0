  ...
  if (notFound || !prop) {
    return (
      <div className="max-w-lg mx-auto text-center py-12 px-4">
        <Building2 size={24} className="text-gold-300 mx-auto mb-4"/>
        <h1 className="font-display text-xl font-bold text-gray-900 dark:text-white mb-2">Listing not found</h1>
        <p className="text-sm text-muted mb-4">It may have been sold, rented, or removed by the seller.</p>
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-4">
      ...
      <div className="relative">
        <div
          ...
          className="h-64 sm:h-80 rounded-lg overflow-x-auto flex snap-x snap-mandatory scroll-smooth no-scrollbar"
        >
          {(prop.imageUrls && prop.imageUrls.length > 0 ? prop.imageUrls : [null]).map((url, i) => (
            <button key={i} type="button" onClick={() => url && setLightboxOpen(true)}
              ...
              {url ? (
                <>
                  <Image src={mediaSrc(url)} alt={`${prop.title} photo ${i + 1}`} fill priority={i === 0} sizes="(max-width: 640px) 100vw, 800px" className="object-contain"/>
              ...
              ) : <Building2 size={22} className="text-gold-300"/>}
            </button>
          ))}
        </div>
        ...
        {prop.imageUrls && prop.imageUrls.length > 1 && (
          <>
            ...
            <div className="absolute bottom-4 right-4 px-2 py-0.5 rounded-md bg-black/50 text-white text-xs font-medium">{activeImage + 1} / {prop.imageUrls.length}</div>
          </>
        )}
      </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {prop.imageUrls.map((url, i) => (
            <button key={url} onClick={() => scrollToImage(i)}
              ...
              <Image src={mediaSrc(url)} alt={`${prop.title} thumbnail ${i + 1}`} fill sizes="64px" className="object-cover"/>
            </button>
          ))}
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="flex flex-wrap gap-3">
            {[
              ...
              <div key={s.label} className="bg-gray-50 dark:bg-[#1A1A35] rounded-lg px-4 py-2.5 text-center min-w-[76px]">
                <p className="text-xs text-muted mb-0.5">{s.label}</p>
                <p className="font-display font-semibold text-gray-900 dark:text-white text-base">{s.value}</p>
              </div>
            ))}
          </div>
          ...
          {prop.description && (
            <Card>
              <h2 className="font-display font-semibold mb-3 text-lg">About this property</h2>
              <p className="text-base text-gray-600 dark:text-gray-300 leading-relaxed whitespace-pre-line">{prop.description}</p>
            </Card>
          )}

          <Card>
            <h2 className="font-display font-semibold mb-3 text-lg flex items-center gap-1.5"><MapPin size={15} className="text-gold-500"/>Location</h2>
          ...
          <Card>
            ...
            {reviews.length === 0 ? (
              <p className="text-sm text-muted text-center py-4">No reviews yet for this property.</p>
            ) : (
        ...
        <div className="space-y-4">
          {trust && (
            <Card>
              <h3 className="font-display font-semibold mb-3 flex items-center gap-2 text-lg"><ShieldCheck size={15} className="text-gold-500"/>Seller trust</h3>
              ...
              <Link href={`/sellers/${prop.sellerId}?propertyId=${prop.id}`} className="mt-3 flex items-center justify-center gap-1.5 h-8 rounded-md border border-gold-300 dark:border-gold-500/30 text-gold-600 dark:text-gold-400 text-sm font-medium hover:bg-gold-50 dark:hover:bg-gold-500/10 transition-colors">
          ...
          {!isSeller && (
            <Card>
              <h3 className="font-display font-semibold mb-1 text-lg">Ready to buy?</h3>
              <p className="text-sm text-muted mb-4">Pay via M-Pesa. Funds sit in escrow until the deal is confirmed. The seller is never paid directly.</p>
              {user ? (
                user.role === 'BUYER' ? (
                  <Button fullWidth onClick={openPayModal} leftIcon={<Wallet size={15}/>}>Pay {fmt.currency(prop.price)}</Button>
                ) : (
                  <p className="text-sm text-muted">Sign in with a buyer account to pay for this property.</p>
                )
              ) : (
          ...
          {!isSeller && (
            <Card>
              <h3 className="font-display font-semibold mb-1 text-lg">Interested?</h3>
              <p className="text-sm text-muted mb-4">Schedule a viewing. A KES {platformConfig.viewingFeeKes} M-Pesa fee confirms your slot.</p>
              {user ? (
                user.role === 'BUYER' ? (
                  <Button fullWidth variant="secondary" onClick={() => setVM(true)} leftIcon={<Calendar size={15}/>}>Schedule viewing</Button>
                ) : (
                  <p className="text-sm text-muted">Sign in with a buyer account to schedule a viewing.</p>
                )
              ) : (
      ...
      <Modal open={viewingModal} onClose={() => setVM(false)} title="Schedule a viewing"
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 text-sm p-2.5 rounded-lg bg-gray-50 dark:bg-white/5">
          ...
          {!prop.fullyTrusted && (
            <label className="flex items-start gap-2 text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10 rounded-lg p-3 cursor-pointer">
      ...
      <Modal open={payModal} onClose={() => setPM(false)} title="Pay for this property"
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between gap-2 text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10 rounded-lg p-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {(['FULL_PAYMENT', 'DEPOSIT'] as const).map(t => (
                <button key={t} type="button" onClick={() => setPF(f => ({ ...f, paymentType: t, amount: t === 'FULL_PAYMENT' ? String(prop.price) : f.amount }))}
                  className={`border-2 rounded-lg p-2.5 text-center text-base font-medium transition-all ${payForm.paymentType === t ? 'border-gold-500 bg-gold-50 dark:bg-gold-500/10 text-gold-600 dark:text-gold-400' : 'border-gray-200 dark:border-[#1E1E3A] text-gray-600 dark:text-gray-300 hover:border-gold-300'}`}>
      ...
      {lightboxOpen && prop.imageUrls && prop.imageUrls.length > 0 && createPortal(
        <div className="fixed inset-0 z-[70] bg-black/90 flex items-center justify-center p-4" onClick={() => setLightboxOpen(false)}>
          ...
          <div onClick={e => e.stopPropagation()} className="relative w-full max-w-5xl h-[80vh]">
            <Image src={mediaSrc(prop.imageUrls[activeImage])} alt={`${prop.title} photo ${activeImage + 1}`} fill sizes="100vw" className="object-contain"/>
          </div>
