    ...
    <div className="space-y-4">
      ...
      {isSeller && !user.verified && (
        <div className="rounded-lg border border-amber-200 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-500/10 px-3 py-2.5 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-base text-amber-900 dark:text-amber-200">Complete your identity verification</p>
            <p className="text-sm text-amber-700 dark:text-amber-400/90 mt-0.5">Verify your identity to activate your listings and build buyer trust.</p>
          </div>
        ...
        <Card className={isSeller ? 'lg:col-span-2' : ''}>
          <div className="flex items-center justify-between mb-4 gap-2">
            ...
            <select value={weeks} onChange={e => setWeeks(Number(e.target.value))} aria-label="Activity period"
              className="text-xs font-medium text-muted bg-transparent border border-base rounded-md pl-2 pr-6 py-1 hover:border-gold-300 dark:hover:border-gold-500/40 focus:outline-none focus:ring-2 focus:ring-gold-500/20 cursor-pointer transition-colors">
        ...
        {isSeller ? (
          <Card>
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-display font-semibold text-base text-gray-900 dark:text-white flex items-center gap-2"><Eye size={14} className="text-muted"/>Top listings</h2>
              <span className="text-2xs text-muted bg-gray-50 dark:bg-white/5 px-2 py-0.5 rounded-full">By views</span>
            </div>
            {loading ? <div className="skeleton h-[180px] rounded-md"/> : topListings.length === 0 ? (
              <div className="h-[180px] flex items-center justify-center text-sm text-muted">No listings yet</div>
            ) : (
              <ResponsiveContainer width="100%" height={180}>
          ...
          <Card>
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-display font-semibold text-base text-gray-900 dark:text-white flex items-center gap-2"><PieChartIcon size={14} className="text-muted"/>Viewing status</h2>
              <span className="text-2xs text-muted bg-gray-50 dark:bg-white/5 px-2 py-0.5 rounded-full">All time</span>
            </div>
            {loading ? <div className="skeleton h-[180px] rounded-md"/> : viewingStatusBreakdown.length === 0 ? (
              <div className="h-[180px] flex items-center justify-center text-sm text-muted">No viewings yet</div>
            ) : (
              <>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card className="card-hover">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display font-semibold text-base text-gray-900 dark:text-white flex items-center gap-2"><Building2 size={14} className="text-muted"/>{isSeller ? 'My listings' : 'Newest listings'}</h2>
            <Link href={isSeller ? '/listings' : '/properties'} className="group text-sm text-gold-500 hover:text-gold-600 font-medium flex items-center gap-1">
            ...
            <div className="space-y-1">
              {props.slice(0,4).map(p => (
                <Link key={p.id} href={`/properties/${p.id}`} className="group flex items-center gap-2.5 p-2 -mx-2 rounded-md hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                  <div className="w-9 h-9 rounded-md bg-gray-50 dark:bg-white/5 text-gold-500 flex items-center justify-center shrink-0 ">
                    <Building2 size={14}/>
                  </div>
                  <div className="flex-1 min-w-0">
        ...
        <Card className="card-hover">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display font-semibold text-base text-gray-900 dark:text-white flex items-center gap-2"><Calendar size={14} className="text-muted"/>Recent viewings</h2>
            <Link href="/viewings" className="group text-sm text-gold-500 hover:text-gold-600 font-medium flex items-center gap-1">
              View all <ArrowRight size={12}/>
            </Link>
          </div>
          {loading ? <div className="space-y-2.5">{[0,1,2].map(i=><div key={i} className="skeleton h-12 rounded-md"/>)}</div>
          : views.length === 0 ? (
            <div className="text-center py-4">
              ...
              <p className="text-sm text-muted">No viewings scheduled</p>
            </div>
          ) : (
            <div className="space-y-1">
              {views.slice(0,4).map(v => (
                <div key={v.id} className="flex items-center gap-2.5 p-2 -mx-2 rounded-md">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 dark:text-white">{fmt.date(v.scheduledAt)}</p>
                    <p className="text-xs text-muted">{fmt.datetime(v.scheduledAt)}</p>
                  </div>
