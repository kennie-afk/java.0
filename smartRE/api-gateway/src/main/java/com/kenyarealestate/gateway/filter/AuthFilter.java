        ...
        return (exchange, chain) -> {
            ...
            return Mono.zip(
                    ...
                    .onErrorResume(ex -> {
                        ...
                        return Mono.just(Optional.<Tuple3<Boolean, Boolean, String>>empty());
                    })
                    .flatMap(flags -> flags
                            .map(f -> onRedisChecksResolved(exchange, chain, token, userId, issuedAtMillis, f))
                            .orElseGet(() -> deny(exchange, HttpStatus.SERVICE_UNAVAILABLE,
