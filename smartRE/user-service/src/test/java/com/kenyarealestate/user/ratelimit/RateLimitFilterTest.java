...
class RateLimitFilterTest {
    ...
    class Scope {
        ...
        void internalExempt() throws Exception {
            MockHttpServletRequest req = new MockHttpServletRequest("POST", "/api/users/internal/sync");
            req.setRequestURI("/api/users/internal/sync");
            filter.doFilter(req, response, chain);
            verifyNoInteractions(limiter);
        }
    ...
    class Buckets {
        ...
        void readAndWriteKeysDiffer() throws Exception {
            MockHttpServletRequest read = new MockHttpServletRequest("GET", "/api/users");
            read.setRequestURI("/api/users");
            read.setRemoteAddr("10.0.0.1");
            MockHttpServletRequest write = new MockHttpServletRequest("DELETE", "/api/users/1");
            write.setRequestURI("/api/users/1");
