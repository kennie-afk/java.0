...
public class StartupChecks implements CommandLineRunner {
    ...
    private void checkInternalSecret() {
        ...
        if (isLocalOrDevProfile()) {
            log.warn("############################################################");
            log.warn("# NOTIFICATION-SERVICE: services.internal-secret {}", reason);
            ...
            log.warn("# requests to notification-service.");
            log.warn("############################################################");
        } else {
            log.error("############################################################");
            log.error("# NOTIFICATION-SERVICE: services.internal-secret {}", reason);
