package com.kenyarealestate.payment.ratelimit;
...
class RateLimitFilterTest {
    ...
    class Scope {
        ...
        void internalExempt() throws Exception {
            MockHttpServletRequest req = new MockHttpServletRequest("POST", "/api/payments/internal/sync");
            req.setRequestURI("/api/payments/internal/sync");
            filter.doFilter(req, response, chain);
            verifyNoInteractions(limiter);
        }
        ...
        void disabled() throws Exception {
            ...
            req.setRequestURI("/api/payments");
            filter.doFilter(req, response, chain);
            verifyNoInteractions(limiter);
        }
    }
    ...
    class Buckets {
        ...
        void readAndWriteKeysDiffer() throws Exception {
            MockHttpServletRequest read = new MockHttpServletRequest("GET", "/api/payments");
            read.setRequestURI("/api/payments");
            read.setRemoteAddr("10.0.0.1");
            MockHttpServletRequest write = new MockHttpServletRequest("DELETE", "/api/payments/1");
            write.setRequestURI("/api/payments/1");
