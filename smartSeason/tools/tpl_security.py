...
/**
 * Publishes domain events to Kafka, keyed by aggregate id so that all events for one
 * aggregate land on the same partition and stay ordered.
 *
 * <p>Publishing is opt-in via {{@code smartseason.events.enabled}}; with it off the service
 * runs standalone (tests, local development) and events are logged instead of sent.
 */
@Component
public class EventPublisher {{
    ...
    /**
     * @param domain  bounded context, e.g. {{@code farm}}
     * @param type    event name, e.g. {{@code FarmCreated}}
     */
