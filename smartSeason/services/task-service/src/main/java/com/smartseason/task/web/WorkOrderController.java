...
import com.smartseason.task.service.WorkOrderService;
...
@RestController
@RequestMapping("/api/task/v1/work-orders")
@Tag(name = "WorkOrder", description = "Work orders, assignments, checklists, photo/GPS evidence, verification")
public class WorkOrderController {
    ...
    public ResponseEntity<WorkOrderResponse> create(@Valid @RequestBody WorkOrderCreateRequest request) {
        ...
        return ResponseEntity.created(URI.create("/api/task/v1/work-orders/" + created.id())).body(created);
    }
    ...
    @Operation(summary = "Delete a WorkOrder")
