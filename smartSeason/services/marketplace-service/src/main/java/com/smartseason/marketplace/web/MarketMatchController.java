...
import com.smartseason.marketplace.service.MarketMatchService;
...
@RestController
@RequestMapping("/api/marketplace/v1/market-matches")
@Tag(name = "MarketMatch", description = "Supply listings, demand posts, offers, buyer-seller matching")
public class MarketMatchController {

    private final MarketMatchService service;

    public MarketMatchController(MarketMatchService service) {
    ...
    public ResponseEntity<MarketMatchResponse> create(@Valid @RequestBody MarketMatchCreateRequest request) {
        ...
        return ResponseEntity.created(URI.create("/api/marketplace/v1/market-matches/" + created.id())).body(created);
    }
