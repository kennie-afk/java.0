...
import com.smartseason.automation.service.ActuatorCommandService;
...
@RestController
@RequestMapping("/api/automation/v1/actuator-commands")
@Tag(name = "ActuatorCommand", description = "Automation rules, actuator command pipeline, safety interlocks, digital twin")
public class ActuatorCommandController {

    private final ActuatorCommandService service;

    public ActuatorCommandController(ActuatorCommandService service) {
    ...
    public ResponseEntity<ActuatorCommandResponse> create(@Valid @RequestBody ActuatorCommandCreateRequest request) {
        ...
        return ResponseEntity.created(URI.create("/api/automation/v1/actuator-commands/" + created.id())).body(created);
    }
