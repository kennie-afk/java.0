...
import com.smartseason.agronomy.service.PestDiseaseService;
...
@RestController
@RequestMapping("/api/agronomy/v1/pest-diseases")
@Tag(name = "PestDisease", description = "Advisories, pest/disease library, scouting reports, crop playbooks")
public class PestDiseaseController {

    private final PestDiseaseService service;

    public PestDiseaseController(PestDiseaseService service) {
    ...
    public ResponseEntity<PestDiseaseResponse> create(@Valid @RequestBody PestDiseaseCreateRequest request) {
        ...
        return ResponseEntity.created(URI.create("/api/agronomy/v1/pest-diseases/" + created.id())).body(created);
    }
