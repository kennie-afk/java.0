...
import com.smartseason.catalog.service.ProductService;
...
@Tag(name = "Product", description = "Produce taxonomy, products, variants, grading standards, certifications")
public class ProductController {
    ...
    @GetMapping
    @Operation(summary = "List products for the caller's tenant")
    ...
    public ResponseEntity<ProductResponse> create(@Valid @RequestBody ProductCreateRequest request) {
        ...
        return ResponseEntity.created(URI.create("/api/catalog/v1/products/" + created.id())).body(created);
    }

    @PatchMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER', 'OPERATOR')")
    @Operation(summary = "Apply a partial update to a Product")
