...
@RestController
@RequestMapping("/api/ledger/v1/accounts")
@Tag(name = "Account", description = "Double-entry accounts, immutable postings, balances, statements")
public class AccountController {
    ...
    public ResponseEntity<AccountResponse> create(@Valid @RequestBody AccountCreateRequest request) {
        ...
        return ResponseEntity.created(URI.create("/api/ledger/v1/accounts/" + created.id())).body(created);
    }

    @PatchMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER', 'OPERATOR')")
    @Operation(summary = "Apply a partial update to a Account")
