...
import com.smartseason.farm.service.SoilProfileService;
...
@RestController
@RequestMapping("/api/farm/v1/soil-profiles")
@Tag(name = "SoilProfile", description = "Farms, plots, geo boundaries, soil profiles, cooperative membership")
public class SoilProfileController {

    private final SoilProfileService service;

    public SoilProfileController(SoilProfileService service) {
    ...
    public ResponseEntity<SoilProfileResponse> create(@Valid @RequestBody SoilProfileCreateRequest request) {
        ...
        return ResponseEntity.created(URI.create("/api/farm/v1/soil-profiles/" + created.id())).body(created);
    }
