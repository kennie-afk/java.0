...
const STORAGE_KEY = "smartseason.nav.collapsed";
        ...
        return (
          <div key={section.key} className="mt-5 first:mt-0">
            <button
              ...
              aria-expanded={open}
              className="flex w-full cursor-pointer items-center justify-between px-2 pb-1.5 text-[0.625rem] font-bold uppercase tracking-[0.16em] text-[var(--color-muted)] opacity-70 transition-opacity duration-150 hover:opacity-100"
            >
              {section.label}
              <svg
                viewBox="0 0 20 20"
                      ...
                      <Link
                        ...
                        className={`relative rounded-md px-2.5 py-1.5 text-sm transition-colors duration-150 ${
                          active
                            ...
                            : "text-[var(--color-muted)] hover:bg-[#f1f4f2] hover:text-[var(--color-ink)]"
                        }`}
                      >
                        {item.label}
                      </Link>
