/**
 * Kenyan administrative areas used by the county / sub-county pickers.
 *
 * The 47 counties are the gazetted list under the 2010 Constitution. The
 * sub-county lists follow constituency boundaries, which is how most counties
 * define their sub-counties in practice - a handful of counties draw internal
 * administrative boundaries that differ, so verify against the county's own
 * schedule before relying on this for statutory reporting.
 */
...
export const SUB_COUNTIES: Record<string, string[]> = {
  ...
  Baringo: [
    "Tiaty", "Baringo North", "Baringo Central", "Baringo South", "Mogotio",
    "Eldama Ravine"
  ],
...
export function subCountiesOf(county: string | null | undefined): string[] {
  if (!county) return [];
